...
function Fp2prod(n, k, a, b, p){
    std::size_t out[2][2][20];
    // solve for X and Y such that a0*b0 + (p-a1)*b1 = p*X + Y with Y in [0,p) 
    // -a1*b1 = (p-a1)*b1 mod p
    std::size_t a0b0_var[20] = prod(n, k, a[0], b[0]);
    std::size_t a1_neg[20] = long_sub(n, k, p, a[1]);
    std::size_t a1b1_neg[20] = prod(n, k, a1_neg, b[1]);
    std::size_t diff[20] = long_add(n, 2*k, a0b0_var, a1b1_neg); // 2*k+1 registers
    ...
    std::size_t a0b1_var[20] = prod(n, k, a[0], b[1]);
    std::size_t a1b0_var[20] = prod(n, k, a[1], b[0]);
    std::size_t sum[20] = long_add(n, 2*k, a0b1_var, a1b0_var); // output 2*k+1 registers
...
template<std::size_t n, std::size_t k> void Fp2multiply(){
    ...
    std::size_t Xvar[2][2][20] = Fp2prod(n, k, a, b, p);
    ...
    for(std::size_t eps=0; eps<2; eps++){
        ...
        for(std::size_t i=k; i<2*k-1; i++)
            carry_check[eps].in[i] <== ab.out[eps][i] - pX[eps].out[i]; 
        for(std::size_t i=2*k-1; i<2*k+3; i++)
...
function prod3D(n, k, l, a, b, c) {
    ...
    std::size_t prod_val[20][20]; // length is 3l - 2 by 3k - 2
    for (int i = 0; i < 3 * k; i++) {
        for (std::size_t j = 0; j < 3 * l; j ++) {
            prod_val[j][i] = 0;
        }
    }
    ...
    std::size_t out[20][20]; // length is 3 * l by 3 * k

    std::size_t split[20][20][3]; // second dimension has length 3 * k - 1
    for (std::size_t j = 0; j < 3 * l - 1; j ++) {
        for (int i = 0; i < 3 * k - 1; i++) {
            split[j][i] = SplitThreeFn(prod_val[j][i], n, n, n);
        }
    }

    std::size_t carry[20][20]; // length is 3l-1 x 3k
...
template<std::size_t n, std::size_t k, std::size_t p> void Fp12MultiplyThree() {
    ...
    std::size_t c0[l][k];
    std::size_t c1[l][k];
    std::size_t neg_a0[l][20];
    std::size_t neg_a1[l][20];
   for (int i = 0; i < l; i ++) {
        for ( std::size_t j = 0; j < k; j ++) {
            a0[i][j] = a[i][0][j];
            a1[i][j] = a[i][1][j];
    ...
    std::size_t real_init[3*l-1][20];
    std::size_t imag_init[3*l-1][20];
    std::size_t imag_init_neg[3*l-1][20];
    std::size_t real[l][2][20];
    std::size_t imag[l][2][20];
    // each product will be 3l-1 x 3k
    std::size_t a0b0c0_var[20][20] = prod3D(n, k, l, a0, b0, c0);
    std::size_t a1b1c0_neg[20][20] = prod3D(n, k, l, neg_a1, b1, c0);
    std::size_t a1b0c1_neg[20][20] = prod3D(n, k, l, neg_a1, b0, c1);
    std::size_t a0b1c1_neg[20][20] = prod3D(n, k, l, neg_a0, b1, c1);

    std::size_t a1b0c0_var[20][20] = prod3D(n, k, l, a1, b0, c0);
    std::size_t a0b1c0_var[20][20] = prod3D(n, k, l, a0, b1, c0);
    std::size_t a0b0c1_var[20][20] = prod3D(n, k, l, a0, b0, c1);
    std::size_t a1b1c1_neg[20][20] = prod3D(n, k, l, neg_a0, b1, c1);

    std::size_t a1b0c0_neg[20][20] = prod3D(n, k, l, neg_a1, b0, c0);
    std::size_t a0b1c0_neg[20][20] = prod3D(n, k, l, neg_a0, b1, c0);
    std::size_t a0b0c1_neg[20][20] = prod3D(n, k, l, neg_a0, b0, c1);
    std::size_t a1b1c1_var[20][20] = prod3D(n, k, l, a0, b1, c1);
    ...
    std::size_t real_carry[l][20];
    std::size_t imag_carry[l][20];
    std::size_t real_final[l][20];
    std::size_t imag_final[l][20];
    std::size_t zeros[20]; // to balance register sizes
    for (int i = 0; i < 20; i ++) {
        zeros[i] = 0;
    }
    for (int i = 0; i < l; i ++) {
    ...
    // reduction mod p
    std::size_t prod_real_temp[l][2][20];
    std::size_t prod_imag_temp[l][2][20];
    ...
    std::size_t X0[l][3 * k - 2];
    std::size_t X1[l][3 * k - 2];
    std::size_t X2[l][3 * k - 2];
    std::size_t Y0[l][3 * k - 2];
    std::size_t Y1[l][3 * k - 2];
    std::size_t Y2[l][3 * k - 2];
    for (int i = 0; i < l; i++) {
	...
	if (3 * k - 2 < 2 * k + 4) {
            for (std::size_t j = k; j < 3 * k - 2; j ++) {
		carry_check[i][0].in[j] <== X0[i][j] + X1[i][j] - Y1[i][j] - Y2[i][j] - Y2[i][j] - p_prod_real0[i].out[j] - prod_real[i][1][j];
		carry_check[i][1].in[j] <== Y0[i][j] + X1[i][j] + Y1[i][j] + X2[i][j] + X2[i][j] - p_prod_imag0[i].out[j] - prod_imag[i][1][j];
            }
            for (std::size_t j = 3 * k - 2; j < 2 * k + 4; j++) {
		carry_check[i][0].in[j] <== - prod_real[i][1][j];
		carry_check[i][1].in[j] <== - prod_imag[i][1][j];
	    }
        } else {
	    for (std::size_t j = k; j < 2 * k + 4; j ++) {
		carry_check[i][0].in[j] <== X0[i][j] + X1[i][j] - Y1[i][j] - Y2[i][j] - Y2[i][j] - p_prod_real0[i].out[j] - prod_real[i][1][j];
		carry_check[i][1].in[j] <== Y0[i][j] + X1[i][j] + Y1[i][j] + X2[i][j] + X2[i][j] - p_prod_imag0[i].out[j] - prod_imag[i][1][j];
            }
            for (std::size_t j = 2 * k + 4; j < 3 * k - 2; j++) {
