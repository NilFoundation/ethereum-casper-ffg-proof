...
template<std::size_t n, std::size_t k, std::size_t p>
void PointOnLine(const std::array<std::array<std::array<std::size_t, 3>, 2>, k> &in) {
    std::size_t LOGK = std::log(std::ceil(k));
    std::size_t LOGK2 = std::log(std::ceil(3 * k * k));
...
// Assume: a, b in [0, 2^n) 
template<std::size_t n, std::size_t k, std::size_t a, std::size_t b, std::size_t p>
void PointOnCurve(const std::array<std::array<std::size_t, 2>, k> &in) {
    std::size_t LOGK = std::log(std::ceil(k));
    std::size_t LOGK2 = std::log(std::ceil(((2 * k - 1) * (k * k + 1))));
...
template<std::size_t n, std::size_t k, std::size_t a, std::size_t p>
void PointOnTangent() {
    signal
    input in[2][2][k];
    ...
    component right = BigMultShortLongUnequal(n, 2 * k - 1, k,
                                              3 * n + 2 * LOGK + 3); // 3k-2 registers < (3*k+1)*k*2^{3n}
    for (std::size_t i = 0; i < 2 * k - 1; i++) {
        if (i == 0)
...
void EllipticCurveAddUnequal() {
    ...
    component cubic = BigMultShortLongUnequal(n, k, 2 * k - 1,
                                              3 * n + 4 + 2 * LOGK); // 3k-2 registers < 3 * k^2 * 2^{3n} )
    for (std::size_t i = 0; i < k; i++)
...
void EllipticCurveDouble() {
    ...
    // precompute lambda 
    std::size_t lamb_num[50] = long_add_mod(n, k, long_a, prod_mod(n, k, long_3, prod_mod(n, k, in[0], in[0], p), p),
                                            p);
...
template<std::size_t n, std::size_t k, std::size_t a1, std::size_t b1, std::size_t p>
void EllipticCurveAdd() {
    signal
    input a[2][k];
    signal
    input aIsInfinity;
...
template<std::size_t n, std::size_t k, std::size_t b, std::size_t x, std::size_t p>
void EllipticCurveScalarMultiply() {
    signal
    input in[2][k];
    signal
    input inIsInfinity;
...
template<std::size_t n, std::size_t k, std::size_t b, std::size_t x, std::size_t p>
void EllipticCurveScalarMultiplyUnequal() {
    signal
    input in[2][k];
    signal
    output out[2][k];
