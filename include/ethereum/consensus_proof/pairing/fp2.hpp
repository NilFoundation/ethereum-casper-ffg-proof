...
#include <ethereum/consensus_proof/pairing/bigint_field_elements_func.hpp>
...
template<std::size_t n, std::size_t k, std::size_t overflow, std::size_t p>
void SignedFp2CarryModP() {
    signal
    input in[2][k];
...
template<std::size_t n, std::size_t k, std::size_t m, std::size_t overflow, std::size_t p>
void SignedFp2CompressCarry() {
    signal
    input in[2][k + m];
    signal
    output out[2][k];
...
template<std::size_t n, std::size_t k, std::size_t overflowa, std::size_t overflowb, std::size_t p>
void SignedFp2Divide() {
    assert(k <= 50);
    signal
    input a[2][k];
    signal
    ...
    std::size_t XY[2][2][50] = get_signed_Fp2_carry_witness(n, k, m, mult.out,
                                                            p); // total value is < 2^{nk} * 2^{n*k + overflowb - n + 1}
    // get a = p*X' + Y'
    ...
    for (std::size_t eps = 0; eps < 2; eps++) {
        for (std::size_t i = 0; i < m; i++) {
            // X'' = X-X'
            X[eps][i] < --XY[eps][0][i] -
                        XY1[eps][0][i]; // each XY[eps][0] is in [-2^n, 2^n) so difference is in [-2^{n+1}, 2^{n+1})
            X_range_checks[eps][i] = Num2Bits(n + 2);
            X_range_checks[eps][i].in =
                    X[eps][i] + (1 << (n + 1)); // X[eps][i] should be between [-2^{n+1}, 2^{n+1})
        }
    }
...
template<std::size_t n, std::size_t k, std::size_t power, std::size_t p>
void Fp2FrobeniusMap() {
    signal
    input in[2][k];
    signal
    output out[2][k];
