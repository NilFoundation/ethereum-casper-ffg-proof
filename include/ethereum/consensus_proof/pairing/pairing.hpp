...
template<std::size_t n, std::size_t k, std::size_t q> void LineFunctionEqual() {
    ...
    std::size_t LOGK2 = log_ceil((3*k+1)*k);
...
template<std::size_t n, std::size_t k, std::size_t kg, std::size_t overflowg, std::size_t q> void Fp12MultiplyWithLineUnequal(){
    ...
    std::size_t XI0 = 1;
    std::size_t LOGK1 = log_ceil(6*k);
    std::size_t LOGK2 = log_ceil(6*k * min(kg, 2*k-1) * 6 * (2+XI0) );
    std::size_t LOGK3 = log_ceil( 6*k * min(kg, 2*k-1) * 6 * (2+XI0) * (k + kg - 1) );
    ...
    for(std::size_t l=0; l<2; l++)for(std::size_t j=0; j<2; j++)for(std::size_t idx=0; idx<k; idx++)
...
template<std::size_t n, std::size_t k, std::size_t q> void LineFunctionEqualFp2() {
    ...
    std::size_t LOGK2 = log_ceil(12*k*k + 1);
...
template<std::size_t n, std::size_t k, std::size_t kg, std::size_t overflowg, std::size_t q> void Fp12MultiplyWithLineUnequalFp2(){
    ...
    std::size_t XI0 = 1;
    std::size_t LOGK1 = log_ceil(12*k);
    std::size_t LOGK2 = log_ceil(12*k * min(kg, 2*k-1) * 6 * (2+XI0) );
    std::size_t LOGK3 = log_ceil( 12*k * min(kg, 2*k-1) * 6 * (2+XI0) * (k + kg - 1) );
    assert( overflowg + 3*n + LOGK3 < 251 );

    component line = SignedLineFunctionUnequalNoCarryFp2(n, k, 2*n + LOGK1); // 6 x 2 x 2k - 1 registers in [0, 12k 2^{2n})
    for(std::size_t i=0; i<2; i++)for(std::size_t j=0; j<2; j++)for(std::size_t l=0; l<2; l++)for(std::size_t idx=0; idx<k; idx++)
...
template<std::size_t n, std::size_t k, std::size_t q> void OptimalAtePairing(){
    ...
    /*for(std::size_t i = 0; i <6; i ++)for(std::size_t j = 0; j < 2; j++)for(std::size_t l = 0; l < k; l++) {
        if (i==0 && j == 0 && l == 0) {
            miller.in[i][j][l] <== 1;
        }
        else {
