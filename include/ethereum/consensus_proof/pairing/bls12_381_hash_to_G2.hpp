#include <ethereum/consensus_proof/pairing/bigint.hpp>
#include <ethereum/consensus_proof/pairing/bigint_func.hpp>
#include <ethereum/consensus_proof/pairing/fp.hpp>
#include <ethereum/consensus_proof/pairing/fp2.hpp>
...
// X1(t) = xi * t^2 * X0(t)
    ...
    component xi3t6 = Fp2MultiplyThree(n, k,
                                       p); // shares a hidden component with xi2t4; I'll let compiler optimize that out for readability
    for (var i = 0; i < 2; i++) {
        for (var idx = 0; idx < k; idx++) {
...
// p^2 + 7
    var
    c1[50] = long_add_unequal(n, 2 * k, 1, prod(n, k, p, p),[7]);
// (p^2 + 7) // 16
    var
    c2[2][50] = long_div2(n, 1, 2 * k - 1, c1,[16]);
...
isInfinity <== den_is_zero[0].out + den_is_zero[1].out - den_is_zero[0].
out *den_is_zero[1]
.
out; // OR gate: if either denominator is 0, output point at infinity
...
var p[50] = get_BLS12_381_prime(n, k);
var x_abs = BLS12381_PARAMETER; // this is abs(x). remember x is negative!
var a[2] =
[0,0];
var b[2] =
[4,4];
...
idx<k;
idx++){
P[i][j][idx] <== in[i][j][idx] +
inIsInfinity *(dummy_point[i][j][idx]
- in[i][j][idx]);
...
idx<k;
idx++)
out[i][j][idx] <== add[4].out[i][j][idx] +
isInfinity *(dummy_point[i][j][idx]
- add[4].out[i][j][idx]);
}
...
component Rp = EllipticCurveAddFp2(n, k,[0, 240],
[1012, 1012], p);
for(
var i = 0;
i<2; i++)for(
...
P.inIsInfinity <== R.isInfinity + Rp.isInfinity - R.
isInfinity *Rp
.
isInfinity;
...
var p[50] = get_BLS12_381_prime(n, k);
var x_abs = BLS12381_PARAMETER;

component is_on_curve = PointOnCurveFp2(n, k,[0, 0],
[4,4], p);
