...
std::array<std::size_t, EXPANDED_LEN> ExpandMessageXMD(const std::array<std::size_t, MSG_LEN> &msg,
    ...
    std::array<std::size_t, 1> i2ospDst = I2OSP<1>(DST_LEN);

    std::array<std::size_t, DST_LEN + 1> dstPrime;
    for (int i = 0; i < DST_LEN; i++) {
        dstPrime[i] = dst[i];
    }
    dstPrime[DST_LEN] = i2ospDst[0];

    std::array<std::size_t, 2> i2ospLibStr = I2OSP<2>(EXPANDED_LEN);
...
HashToField(const std::array<std::size_t, MSG_LEN> &msg) {
    ...
    std::size_t LOG_EXTRA = std::ceil(std::log2(NUM_REGISTERS - 6));
    std::size_t tmp;
