...
template Step() {
  ...
  component commitToPublicInputs =
      CommitToPublicInputsForStep(TRUNCATED_SHA256_SIZE);
  for (var i = 0; i < 32; i++) {
  ...
  component verifySignature = VerifySyncCommitteeSignature(
      SYNC_COMMITTEE_SIZE, LOG_2_SYNC_COMMITTEE_SIZE, N, K);
  ...
  component verifyFinality =
      SSZRestoreMerkleRoot(FINALIZED_HEADER_DEPTH, FINALIZED_HEADER_INDEX);
  for (var i = 0; i < 32; i++) {
  ...
  component verifyExecutionState = SSZRestoreMerkleRoot(
      EXECUTION_STATE_ROOT_DEPTH, EXECUTION_STATE_ROOT_INDEX);
  for (var i = 0; i < 32; i++) {
