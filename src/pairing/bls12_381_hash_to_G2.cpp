...
template OptSimpleSWU2(n, k){
    ...
    std::size_t a[2] = [0, 240];
    std::size_t b[2] = [1012, 1012];
    ...
    */
    ...
    // p^2 + 7
    std::size_t c1[50] = long_add_unequal(n, 2*k, 1, prod(n, k, p, p), [7]);
    // (p^2 + 7) // 16
    std::size_t c2[2][50] = long_div2(n, 1, 2*k-1, c1, [16]);

    assert( c2[1][0] == 0 ); // assert p^2 + 7 is divisible by 16

    std::size_t sqrt_candidate[2][50] = find_Fp2_exp(n, k, gX0.out, p, c2[0]);
    ...
    std::size_t roots[4][2][50] = get_roots_of_unity(n, k);
    std::size_t sqrt_witness[2][2][50];
    for(std::size_t i=0; i<2; i++)for(std::size_t j=0; j<2; j++)for(std::size_t idx=0; idx<k; idx++)
        sqrt_witness[i][j][idx] = 0;

    std::size_t is_square = 0;
    for(std::size_t i=0; i<4; i++){
        std::size_t sqrt_tmp[2][50] = find_Fp2_product(n, k, sqrt_candidate, roots[i], p);
    ...
    std::size_t is_square1 = 0;
    std::size_t etas[4][2][50] = get_etas(n, k);
    ...
    std::size_t t_cu[2][50] = find_Fp2_product(n, k, find_Fp2_product(n, k, in, in, p), in, p);
    sqrt_candidate = find_Fp2_product(n, k, sqrt_candidate, t_cu, p);
    
    for(std::size_t i=0; i<4; i++){
        std::size_t sqrt_tmp[2][50] = find_Fp2_product(n, k, sqrt_candidate, etas[i], p);
...
template Iso3Map(n, k){
    ...
    std::size_t coeffs[4][4][2][50] = get_iso3_coeffs(n, k);
    ...
    for(std::size_t i=0; i<2; i++)for(std::size_t idx=0; idx<2*k-1; idx++){
        xp3_nocarry.a[i][idx] <== xp2_nocarry.out[i][idx];
        xp2.in[i][idx] <== xp2_nocarry.out[i][idx];
    }
    for(std::size_t i=0; i<2; i++)for(std::size_t idx=0; idx<3*k-2; idx++)
    ...
    std::size_t deg[4] = [3, 1, 3, 2];
    for(std::size_t i=0; i<4; i++)for(std::size_t j=0; j<deg[i]; j++){
    ...
    std::size_t x_frac[4][2][50];
    for(std::size_t i=0; i<4; i++){
        for(std::size_t l=0; l<2; l++)for(std::size_t idx=0; idx<2*k-1; idx++){
            if(idx<k)
                x_frac[i][l][idx] = coeffs[i][0][l][idx];
            else
                x_frac[i][l][idx] = 0;
        }
        for(std::size_t j=0; j<deg[i]; j++)for(std::size_t l=0; l<2; l++)for(std::size_t idx=0; idx<2*k-1; idx++)
...
template ClearCofactorG2(n, k){
    ...
    std::size_t p[50] = get_BLS12_381_prime(n, k);
    std::size_t x_abs = get_BLS12_381_parameter(); // this is abs(x). remember x is negative!
    std::size_t a[2] = [0,0];
    std::size_t b[2] = [4,4];
    std::size_t dummy_point[2][2][50] = get_generator_G2(n, k);
...
template SubgroupCheckG2(n, k){
    ...
    std::size_t p[50] = get_BLS12_381_prime(n, k);
    std::size_t x_abs = get_BLS12_381_parameter();
