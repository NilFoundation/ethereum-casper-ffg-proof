...
include "bigint.cpp";
include "field_elements_func.cpp";
include "fp.cpp";
include "fp2.cpp";
...
template Fp12FrobeniusMap(n, k, power){
    ...
    std::size_t FP12_FROBENIUS_COEFFICIENTS[12][6][2][20] = get_Fp12_frobenius(n, k);
    std::size_t pow = power % 12;
...
template SignedFp12ScalarMultiplyNoCarry(n, k, m_out){
    ...
    component ab[6][2]; 
    for(std::size_t i=0; i<6; i++)for(std::size_t j=0; j<2; j++){
...
template SignedFp12ScalarMultiplyNoCarryUnequal(n, ka, kb, m_out){
    ...
    for(std::size_t i=0; i<6; i++)for(std::size_t j=0; j<2; j++){
        ...
        for(std::size_t idx=0; idx<ka; idx++)
            ab[i][j].a[idx] <== a[idx];
        for(std::size_t idx=0; idx<kb; idx++)
            ab[i][j].b[idx] <== b[i][j][idx]; 
    }
    
    for(std::size_t i=0; i<6; i++)for(std::size_t j=0; j<2; j++)for(std::size_t idx=0; idx<ka+kb-1; idx++)
        out[i][j][idx] <== ab[i][j].out[idx];
}
...
template SignedFp12Fp2MultiplyNoCarry(n, k, m_out){
    ...
    for(std::size_t i=0; i<6; i++)for(std::size_t idx=0; idx<2*k-1; idx++) {
...
template SignedFp12Fp2MultiplyNoCarryUnequal(n, ka, kb, m_out){
    ...
    for(std::size_t i=0; i<6; i++)for(std::size_t idx=0; idx<ka+kb-1; idx++){
        out[i][0][idx] <== ab[i][0].out[idx] - abi[i][1].out[idx];
        out[i][1][idx] <== abi[i][0].out[idx] + ab[i][1].out[idx];
    }
}
...
template SignedFp12MultiplyNoCarryUnequal(n, ka, kb, m_out){
    ...
    for (int i = 0; i < l; i++)for (std::size_t j = 0; j < ka + kb - 1; j++) {
        if (i < l - 1) {
...
template SignedFp12MultiplyNoCarry(n, k, m_out){
    ...
    for(std::size_t i=0; i<l; i++)for(std::size_t j=0; j<2; j++)for(std::size_t idx=0; idx<k; idx++){
        mult.a[i][j][idx] <== a[i][j][idx];
        mult.b[i][j][idx] <== b[i][j][idx];
    }
    for(std::size_t i=0; i<l; i++)for(std::size_t j=0; j<2; j++)for(std::size_t idx=0; idx<2*k-1; idx++)
...
template Fp12Compress(n, k, m, p, m_out){
    ...
    for (int i = 0; i < l; i++)for (std::size_t j = 0; j < 2; j++){
        reduce[i][j] = PrimeReduce(n, k, m, p, m_out);
        for (std::size_t idx = 0; idx < k + m; idx++)
            reduce[i][j].in[idx] <== in[i][j][idx];
    }

    for (int i = 0; i < l; i++)for (std::size_t j = 0; j < 2; j++)for (std::size_t idx = 0; idx < k; idx++)
...
template SignedFp12MultiplyNoCarryCompress(n, k, p, m_in, m_out) {
    ...
    std::size_t LOGK1 = log_ceil(6*k*(2+XI0));
...
template SignedFp12CarryModP(n, k, overflow, p) {
    ...
    component carry[l][2];
    for(std::size_t i=0; i<l; i++)for(std::size_t j=0; j<2; j++){
...
template Fp12Multiply(n, k, p) {
    ...
    std::size_t LOGK2 = log_ceil(6*k*k*(2+XI0));
...
template Fp12Square(n, k, p) {
    ...
    for(std::size_t i=0; i<6; i++)for(std::size_t j=0; j<k; j++){
...
template Fp6Invert(n, k, p) {
    ...
    signal input a1[2][k];
    signal input a2[2][k];
    std::size_t out[6][2][50] = find_Fp6_inverse(n, k, p, a0, a1, a2);
...
template Fp12Invert(n, k, p){
    ...
    std::size_t inverse[6][2][50] = find_Fp12_inverse(n, k, p, in); // 6 x 2 x 50, only 6 x 2 x k relevant
    for (int i = 0; i < 6; i ++) {
        for (std::size_t j = 0; j < 2; j ++) {
            for (std::size_t m = 0; m < k; m ++) {
    ...
    for(std::size_t i=0; i<6; i++) for(std::size_t j=0; j<2; j++) for(std::size_t m=0; m<k; m++) {
...
template Fp12Exp(n, k, e, p) {
    ...
    for(std::size_t i=0; i<BITLENGTH; i++){
        ...
        if( i > 0 ){ // pow2[0] is never defined since there is no squaring involved
            ...
            for(std::size_t j=0; j<k; j++) pow2[i].p[j] <== p[j];
            if( i == 1 ){
