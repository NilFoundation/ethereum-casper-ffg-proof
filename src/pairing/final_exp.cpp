...
template Fp12CyclotomicCompress(n, k) {
    ...
    for(std::size_t eps=0; eps<2; eps++)for(std::size_t j=0; j<k; j++){
...
template Fp12CyclotomicDecompress(n, k, p) {
    ...
    std::size_t LOGK2 = log_ceil( ((1+XI0)*2*k + 6*k )*k + 1);
    std::size_t LOGK3 = log_ceil( (1+XI0) * 6*k*k + 1 );
    assert(3*n + LOGK2 < 250);

    std::size_t len = 2*k-1; // number of registers in output of Fp2MultiplyNoCarry
    ...
    for(std::size_t i=0; i<k; i++)for(std::size_t eps=0; eps<2; eps++){
        g5sq.a[eps][i] <== in[3][eps][i];
        g5sq.b[eps][i] <== in[3][eps][i];
    }
    // c = XI0 + u
    // g5^2 * c
    std::size_t g5sqc[4][50] = signed_Fp2_mult_w6(len, g5sq.out, XI0); // overflow < (1+XI0)*2k*2^{2n}
    // 3 g4^2
    ...
    for(std::size_t i=0; i<len; i++)for(std::size_t eps=0; eps<2; eps++){
        if(i < k)
    ...
    std::size_t temp[2][len];
    for(std::size_t i=0; i<2; i++)for(std::size_t j=0; j<len; j++)
        temp[i][j] = twog1sq.out[i][j] + g2g5.out[i][j] - threeg3g4.out[i][j];
    
    // (2 g1^2 + g2 g5 - 3 g3 g4)(1+u)
    std::size_t tempc[2][50] = signed_Fp2_mult_w6(len, temp, XI0); // overflow abs val < (1+XI0)*6k * 2^{2n}
...
template SignedFp12CyclotomicSquareNoCarry(n, k) {
    ...
    // c*g3 = (1+u)*g3
    std::size_t cg3[2][50] = signed_Fp2_mult_w6(k, in[1], XI0);
    std::size_t cg5[2][50] = signed_Fp2_mult_w6(k, in[3], XI0);
    for(std::size_t i=0; i<2; i++)for(std::size_t j=0; j<k; j++){
    ...
    std::size_t cB45[2][50] = signed_Fp2_mult_w6(2*k-1, B45.out, XI0);  // (1+XI0)*2k*2^{2N}
    std::size_t cB23[2][50] = signed_Fp2_mult_w6(2*k-1, B23.out, XI0);
    for(std::size_t i=0; i<2; i++)for(std::size_t j=0; j<2*k-1; j++){
        if(j < k){
...
template Fp12CyclotomicSquare(n, k, p) {
    ...
    std::size_t XI0 = 1;
    std::size_t LOGK2 = log_ceil(18*(2+XI0)*k *k + 1);
    assert(3*n + LOGK2 < 251);

    component sq = SignedFp12CyclotomicSquareNoCarry(n, k);
    for(std::size_t i=0; i<4; i++)for(std::size_t eps=0; eps<2; eps++)for(std::size_t j=0; j<k; j++){
    ...
    for(std::size_t i=0; i<4; i++){
        ...
        for(std::size_t eps=0; eps<2; eps++)for(std::size_t j=0; j<2*k-1; j++)
...
template Fp12CyclotomicExp(n, k, e, p) {
    ...
    for(std::size_t i=0; i<6; i++)for(std::size_t eps=0; eps<2; eps++)for(std::size_t j=0; j<k; j++)
    ...
    signal first[6][2][k];
    std::size_t curid = 0; // tracks current index in mult[]
    ...
    for(std::size_t i=0; i<BITLENGTH; i++){
        ...
        if( i > 0 ){ // pow2[0] is never defined since there is no squaring involved
            ...
            if( i == 1 ){
                for(std::size_t id=0; id<4; id++)for(std::size_t eps=0; eps<2; eps++)for(std::size_t j=0; j<k; j++)
        ...
        if( ((e >> i) & 1) == 1 ){
            ...
            if(curid == 0){ // this is the least significant bit
                if( i == 0 ){
                    for(std::size_t id=0; id<6; id++)for(std::size_t eps=0; eps<2; eps++)for(std::size_t j=0; j<k; j++)
